...
const Dashboard = () => {
    ...
    /* 
        const { data: products = [], } = useQuery({
            queryKey: ['products'],
            queryFn: async () => {
                const res = await fetch("https://resale-market-server-wahid137.vercel.app/dashboard/myproduct", {
                    headers: {
                        authorization: `bearer ${localStorage.getItem('accessToken')}`
                    }
                })
        ...
        <div>
            <div className='p-7'>
                ...
                <div className='bg-warning p-5'>
                    ...
                    <div className="w-full max-w-sm mb-5">
                        <label className="label">
                            <span className="label-text text-accent font-bold">Total Product: {/* {products.length} */}</span>
                        </label>
                        <input type="range" min={0} max="100" value="10"/* {products.length}  */ className="range range-secondary" />
                    </div>
                    <div className="w-full max-w-sm mb-5">
                        <label className="label">
