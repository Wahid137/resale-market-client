...
const MyProductCard = ({ product, refetch }) => {
    const { productName, image, type, time, year, resalePrice, originalPrice, situation } = product
        ...
        <div className='bg-white overflow-hidden rounded-lg shadow-md hover:shadow-xl'>
            ...
            <div className='p-2'>
                ...
                <div className='flex flex-col pb-2'>
                    ...
                    {
                        situation !== "advertised" && <button onClick={() => handleUpdateProduct(product)} className='btn btn-sm w-1/2 mx-auto btn-secondary'>Advertise</button>
                    }
                    {
                        situation === "advertised" && <button onClick={() => handleUpdateProduct(product)} className='btn btn-sm w-1/2 mx-auto btn-accent'>Advertised</button>
                    }
                </div>
            </div>
            {
                deletingProduct && <ConfirmationModal
                    ...
                    successAction={handleDeleteProduct} //pass the function
